# ⚡ RAG Guardrails Action (GitHub Action)

> Vulnerability assessment action auditing repo files for context poisoning vulnerabilities.
...
## 🎛️ How It Works
Open the **[Interactive Studio](https://pradeeptalari14.github.io/portfolio/tools/rag-guardrails-action/)** in your portfolio dashboard to customize options, select webhook notification channels (Slack/Teams), toggle chat bot connections (WhatsApp Business/Telegram Bot), and download compiled deployment scripts dynamically.
...
```bash
git clone https://github.com/Pradeeptalari14/tp-rag-guardrails-action.git
cd tp-rag-guardrails-action
```
...
```bash
kubectl port-forward deployment/tp-rag-guardrails-action 8080:8080 || echo "Port forwarding enabled on localhost:8080"
```
...
└── primary_config/
    └── action.yml     # Main configuration file
```
...
---
*Generated by [RAG Guardrails Action (GitHub Action) Studio](https://pradeeptalari14.github.io/portfolio/tools/rag-guardrails-action/) — [Talari Pradeep SRE Portfolio](https://pradeeptalari14.github.io/portfolio)*
